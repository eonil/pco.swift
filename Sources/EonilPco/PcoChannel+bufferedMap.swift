...
public extension PcoIncomingChannel {
    ///
    ...
    /// a signal.
    ///
    public func bufferedMap<U>(_ bufferedMap: @escaping (Signal) -> ([U])) -> PcoAnyIncomingChannel<U> {
        let ch1 = PcoBufferedIncomingChannel(underlyingChannel: self, bufferedMap)
        let ch2 = PcoAnyIncomingChannel<U>(ch1)
        return ch2
    }
}
public extension PcoOutgoingChannel {
    ///
    ...
    /// a signal.
    ///
    public func bufferedMap<U>(_ bufferedMap: @escaping (U) -> ([Signal])) -> PcoAnyOutgoingChannel<U> {
